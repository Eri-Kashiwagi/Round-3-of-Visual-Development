...
namespace WSC_SimChallenge_2024_Net.PortSimulation
{
    public class PortSimModel : Sandbox
    {
        ...
        public void QuaySideGenerator(int numberofBerths)
        {
            ...
            for (int id = 0; id < numberofBerths; id++)
            {
                ...
                for (int q = 0; q < NumberofQCsPerBerth; q++)
                {
                    ...
                    QC qc = new QC {Id =$"qc{id* NumberofQCsPerBerth+q}", CP = cp,status="QC"};
                    QCs.Add(qc);
                    qcs.Add(qc);
        ...
        public void QCAGVS()
        {
            ...
            {
                Console.Write($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")},{qc.status},");
                foreach (var agv in qc.agvs)
                {
                    Console.Write($"{agv.Id},");
                }
                Console.WriteLine();
            }
        }
