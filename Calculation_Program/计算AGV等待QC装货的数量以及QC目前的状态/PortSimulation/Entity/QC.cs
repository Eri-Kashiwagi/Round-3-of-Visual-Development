...
namespace WSC_SimChallenge_2024_Net.PortSimulation.Entity
{
    public class QC
    {
        ...
        public class BeingIdle: BaseActivity<QC>
        {
            ...
            public override void RequestToStart(QC load)
            {
        ...
        public class RestoringtoDischarge : BaseActivity<QC>
        {
            ...
            {
                if (_debugMode) Console.WriteLine($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")}  {ActivityName}.Start({load})");
                load.status = "RestoringtoDischarge";
                HourCounter.ObserveChange(1);
                ProcessingList.Add(load);
                PendingList.Remove(load);
                ReadyToStartList.Remove(load);
        ...
        public class HoldingonDischarging : BaseActivity<QC>
        {
            ...
            {
                if (_debugMode) Console.WriteLine($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")}  {ActivityName}.Start({load})");
                load.status = "HoldingonDischarging";
                HourCounter.ObserveChange(1);
                ProcessingList.Add(load);
                PendingList.Remove(load);
                ReadyToStartList.Remove(load);
