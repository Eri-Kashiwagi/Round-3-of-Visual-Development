...
namespace WSC_SimChallenge_2024_Net.PortSimulation
{
    public class PortSimModel : Sandbox
    {
            ...
            for (int id = 0; id < numberofBerths; id++)
            {
                ...
                for (int q = 0; q < NumberofQCsPerBerth; q++)
                {
                    ...
                    for (int a = 0; a < NumberofInitialAGVsPerQC; a++)
                    {
                        ...
                        agv.ChangeLocation = new ControlPoint();
                        agv.UsefulLocation = new ControlPoint();
                        agv.ChangeLocation.Xcoordinate = cp.Xcoordinate;
                        agv.ChangeLocation.Ycoordinate = cp.Ycoordinate;
                        agv.UsefulLocation.Xcoordinate = cp.Xcoordinate;
                        agv.UsefulLocation.Ycoordinate = cp.Ycoordinate;
                        AGVs.Add(agv);
        ...
        public void GetAGVs()
        {
            ...
            {
                //Console.WriteLine($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")} {agv.UsefulLocation.Xcoordinate} {agv.UsefulLocation.Ycoordinate}");
                if (agv.flag == 0) Console.WriteLine($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")},{agv.UsefulLocation.Xcoordinate},{agv.UsefulLocation.Ycoordinate},{agv.flag},");
                else if (agv.flag == 1) Console.WriteLine($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")},{agv.UsefulLocation.Xcoordinate},{agv.UsefulLocation.Ycoordinate},{agv.flag},{agv.LoadedContainer.DischargingVesselID}");
                else Console.WriteLine($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")},{agv.UsefulLocation.Xcoordinate},{agv.UsefulLocation.Ycoordinate},{agv.flag},{agv.LoadedContainer.LoadingVesselID}");
            }
        }
