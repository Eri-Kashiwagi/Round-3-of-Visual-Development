    ...
    public class PortSimModel : Sandbox
    {
        ...
        public void YardGenerator()
        {
            ...
            for (int i = 0; i < numberofYCs; i++)
            {
                ...
                YC yC = new YC { Id = "YC" + i.ToString(), ServedBlock = yardBlock, CP = controlPoint,status="YC"};
        ...
        public void YCAGVS()
        {
            foreach (var yb in YardBlocks)
            {
                Console.Write($"{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")},{yb.ContainerNum},{yb.EquipedYC.status},");
                foreach (var agv in yb.agvs)
                {
                    Console.Write($"{agv.Item1.Id},{agv.Item2},");
                }
                Console.WriteLine();
            }
        }
