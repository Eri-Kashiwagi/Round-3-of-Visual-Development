...
void AGV_waitlist_container_count_per_yard_Status::unzip_use_file(int x) {
    if (file.is_open()) {
        file.close();
    }
    for (int i = 1; i <= 70; i++) {
...
void AGV_waitlist_container_count_per_yard_Status::unzip_file(int x) {
    x += 1;
    if (x > 71 || x < 1)return;
    string zipFileName = "Calculated_Data\\AGV_waitlist_container_count_per_yard_Status" + to_string(x) + ".zip";
    char zipFullPath[MAX_PATH];
...
void AGV_waitlist_container_count_per_yard_Status::use_file(int x) {
