    ...
    public class Container
    {
            ...
            public void GenerateContainers(QCLine qCLine)//Connect QCLine.Dicharging.OnStart
            {
                ...
                for (int dest = 0; dest < servedVessel.DischargingContainersInformation.Count; dest++)
                {
                    ...
                    for (int i = 0; i < servedVessel.DischargingContainersInformation["vessel " + dest.ToString()]; i++)
                    {
                        ...
                        {
                            Id = id,
                            DischargingVessel = servedVessel,
        ...
        public class TransportingToYard : BaseActivity<Container>
        {
            ...
            public override void TryStart(Object obj)
            {
                ...
                else { Console.WriteLine($"Error1：{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")}  {ActivityName}.TryStart({obj}): we don't find the matched container"); }
        ...
        public class BeingStacked : BaseActivity<Container>
        {
            ...
            public override void TryStart(Object obj)
            {
                ...
                else { Console.WriteLine($"Error11：{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")}  {ActivityName}.TryStart({obj}): we don't find the matched container"); }
            }
                    ...
                    foreach (var infor in qcLine.LoadingContainersInformation)
                    {
                        for (int i = 0; i < infor.Value; i++)
                        {
                            var stockedContainer = CompletedList
                                    .Where(container => container.LoadingVesselID == qcLine.ServedVessel.Id && container.Week < qcLine.ServedVessel.Week) // Filter by LoadingVesselID
                                    .OrderBy(container => container.Week) // Order by arrivaltime
                                    .FirstOrDefault(); // Take the first one, which will be the one with the earliest arrival time

                            if (stockedContainer != null)
                            {
                                //Console.WriteLine($"{qcLine.ServedVessel.Id}, {qcLine.ServedVessel.ArrivalTime}, {infor.Key}, {infor.Value}");
                            ...
                            {
                                Console.WriteLine($"{qcLine.ServedVessel.Id}, {infor.Key}, {infor.Value}");
                                Console.WriteLine($"Error：{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")}  {ActivityName}.TryFinish({obj}): we don't have containers from {infor.Key} to {qcLine.ServedVessel.Id}");
                            }
                        }
                    }
                }
        ...
        public class TransportingToQuaySide : BaseActivity<Container>
        {
            ...
            public override void TryStart(Object obj)
            {
                ...
                else { Console.WriteLine($"Error3：{ClockTime.ToString("yyyy-MM-dd HH:mm:ss")}  {ActivityName}.TryStart({obj}): we don't find the matched container"); }
